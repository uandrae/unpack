...
class Experiment:
 ...
 def __init__(self, name=None, path=None, mbrs=None):
        self.name = name
...
def find_files(path,pattern):
  ...
  for entry in it:
      if not entry.name.startswith('.') and entry.is_file():
          if re.search('(.*)'+pattern+'\d{10}.tar.gz',entry.name) :
            result.append(entry.name)
      if not entry.name.startswith('.') and entry.is_dir():
...
def run(config) :
  ...
  for i,e in enumerate(config['experiments']):
    ...
    exp[i] = Experiment(e,config['experiments'][e]['input_path'],members)
